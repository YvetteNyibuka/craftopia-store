...
export interface ApiProduct {
    ...
    images: string[];
    category: string;
    collectionId?: { _id: string; name: string; slug: string };
    price: number;
    comparePrice?: number;
    sku?: string;
    stockCount: number;
...
export const productsApi = {
    list: (params?: {
        ...
        category?: string; collection?: string; sort?: string; order?: string; status?: string;
    }) =>
    ...
    lowStock: (threshold?: number) =>
        api.get<{ status: string; data: ApiProduct[]; total: number }>("/products/low-stock", {
    ...
    update: (id: string, data: Partial<ApiProduct>) =>
        api.patch<{ status: string; data: ApiProduct }>(`/products/${id}`, data),
...
export const collectionsApi = {
    ...
    update: (id: string, data: Partial<ApiCollection>) =>
        api.patch<{ status: string; data: ApiCollection }>(`/collections/${id}`, data),
