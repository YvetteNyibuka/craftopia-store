...
export default function CartPage() {
    const { items, subtotal, total, itemCount, updateQuantity, removeItem, clearCart, fetchCart, mergeGuestCart, isLoading } = useCartStore();
    ...
    // Sync cart with server when auth changes
    useEffect(() => {
        if (!mounted) return;
        if (isAuthenticated) {
            mergeGuestCart();
        }
    }, [isAuthenticated, mounted, mergeGuestCart]);
    ...
    return (
        <div className="container mx-auto px-4 py-12 min-h-screen max-w-6xl">
            <div className="flex items-center justify-between mb-8">
                <h1 className="font-serif text-3xl font-medium text-[#111]">
                    Shopping Cart {itemCount > 0 && <span className="text-stone-400 text-xl font-normal">({itemCount})</span>}
                </h1>
                {items.length > 0 && (
                    <button onClick={() => clearCart()} className="text-[13px] text-stone-400 hover:text-red-500 font-semibold transition-colors">
                        Clear cart
                    </button>
                )}
            </div>
                    ...
                    <div className="w-full lg:w-2/3">
                        <div className="bg-white rounded-3xl border border-stone-100 overflow-hidden shadow-sm">
                            ...
                            <ul className="divide-y divide-stone-100">
                                {items.map((item) => (
                                    <li key={item.id} className="grid grid-cols-12 gap-4 items-center p-6 bg-white">
                                        <div className="col-span-6 flex items-center gap-4">
                                            ...
                                            <div>
                                                <Link href={item.slug ? `/shop/${item.slug}` : "#"} className="font-semibold text-sm text-[#111] hover:text-primary transition-colors line-clamp-2">
                                                    {item.title}
                                                </Link>
                                                {item.variant && <p className="text-stone-400 text-xs mt-0.5">{item.variant}</p>}
                                                <p className="text-stone-500 text-xs mt-1">{formatCurrency(item.price)}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="col-span-3 flex justify-end items-center gap-4">
                                            <span className="font-bold text-[15px] text-[#111]">{formatCurrency(item.price * item.quantity)}</span>
                                            <button
                        ...
                        <div className="bg-[#FAF9F6] rounded-3xl p-8 border border-stone-100 shadow-sm sticky top-24">
                            ...
                            <div className="space-y-4 mb-6 text-sm text-stone-600 font-medium">
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-semibold text-[#111]">{formatCurrency(tax)}</span>
                                </div>
                            </div>
                            ...
                            <div className="border-t border-stone-200 pt-5 mb-8 flex justify-between items-end">
                                ...
                                <div className="text-right">
                                    ...
                                    <span className="font-bold text-3xl text-[#5CE614] leading-none">{formatCurrency(orderTotal)}</span>
                                </div>
                            </div>
