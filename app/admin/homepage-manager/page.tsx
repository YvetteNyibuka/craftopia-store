...
import { homepageApi, HomepageContent } from "@/lib/api/homepage";
...
export default function ContentManagerPage() {
  ...
  const updateBanner = (
    ...
    setBanners((bs) =>
      bs.map((b) => (b.id === id ? { ...b, [field]: value } : b)),
    );
  ...
  const toggleColVisibility = (id: string) =>
    setFeaturedCols((cs) =>
      cs.map((c) => (c.id === id ? { ...c, visible: !c.visible } : c)),
    );
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setHomepageId(res.data._id);
        setAnnouncementEnabled(res.data.announcementEnabled);
        setAnnouncementText(res.data.announcementText || "");
        setBanners(
          res.data.heroBanners.map((b) => ({
            id: b._id || `b${Date.now()}`,
            heading: b.heading,
        ...
        setFeaturedCols(
          res.data.featuredCollections.map((c) => ({
            id: c._id || `c${Date.now()}`,
            name: c.name,
            productCount: 0,
            visible: c.visible,
  ...
  const handleSave = async () => {
    ...
    try {
      await homepageApi.update({
        ...
        featuredCollections: featuredCols.map((c, i) => ({
          collectionId: c.id,
          name: c.name,
          visible: c.visible,
          order: i,
  ...
  const inputCls =
    "flex h-11 w-full rounded-xl border border-stone-200 bg-white px-3 py-2 text-sm shadow-sm transition-colors placeholder:text-stone-400 focus:outline-none focus:ring-2 focus:ring-[#5CE614] focus:border-[#5CE614]";
    ...
    <main className="max-w-[900px] space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-[14px] text-stone-500 mt-1">
            Edit your homepage layout, banners, and section visibility.
          </p>
        </div>
      ...
      >
        ...
        {announcementEnabled && (
          <FormField
            ...
            hint="Keep it short — ~80 characters works best."
          >
            <input
      ...
      >
        ...
        <div className="space-y-4">
          {banners.map((banner, idx) => (
            <div
              key={banner.id}
              className="bg-stone-50 rounded-2xl border border-stone-100 p-5 space-y-4"
            >
              <div className="flex items-center justify-between">
              ...
              <div className="h-28 border-2 border-dashed border-stone-200 rounded-xl flex items-center justify-center text-stone-400 hover:border-[#5CE614] hover:bg-[#F0FBE8]/30 transition-all cursor-pointer group">
                <div className="text-center">
                  ...
                  <p className="text-[11px] font-medium group-hover:text-[#5CE614] transition-colors">
                    Upload banner image
                  </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <FormField label="Heading">
                  <input
                    ...
                    onChange={(e) =>
                      updateBanner(banner.id, "heading", e.target.value)
                    }
                ...
                <FormField label="Subheading">
                  <input
                    ...
                    onChange={(e) =>
                      updateBanner(banner.id, "subheading", e.target.value)
                    }
                ...
                <FormField label="CTA Button Label">
                  <input
                    ...
                    onChange={(e) =>
                      updateBanner(banner.id, "ctaLabel", e.target.value)
                    }
                ...
                <FormField label="CTA Link">
                  <input
                    ...
                    onChange={(e) =>
                      updateBanner(banner.id, "ctaLink", e.target.value)
                    }
          ...
          {[
            {
              ...
              icon: ImageIcon,
              label: "Hero Banner",
              desc: "Full-width top banner",
            },
            {
              key: "featuredEnabled",
              icon: Star,
              label: "Featured Collections",
              desc: "Collection grid tiles",
            },
            {
              key: "bestSellersEnabled",
              icon: BadgeCheck,
              label: "Best Sellers Strip",
              desc: "Most-purchased products",
            },
            {
              key: "reviewsEnabled",
              icon: Star,
              label: "Customer Reviews",
              desc: "Star ratings & testimonials",
            },
            {
              key: "newsletterEnabled",
              icon: Bell,
              label: "Newsletter Popup",
              desc: "Email capture modal",
            },
            {
              key: "instagramEnabled",
              icon: Instagram,
              label: "Instagram Feed",
              desc: "Social media grid",
            },
            {
              key: "sustainabilityEnabled",
              icon: ChevronDown,
              label: "Sustainability Banner",
              desc: "Eco-credentials section",
            },
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`w-8 h-8 rounded-xl flex items-center justify-center ${isOn ? "bg-[#E9F4E5]" : "bg-stone-100"} transition-colors`}
                  >
                    <Icon
                      className={`w-4 h-4 ${isOn ? "text-[#5CE614]" : "text-stone-400"} transition-colors`}
                    />
                  </div>
                  <div>
      ...
      <div className="sticky bottom-0 bg-white/90 backdrop-blur-sm border-t border-stone-100 -mx-8 px-8 py-4 flex justify-between items-center">
        <p className="text-[12px] text-stone-400">
          Changes are saved to a draft until you publish.
        </p>
        <button
          type="button"
          onClick={handleSave}
