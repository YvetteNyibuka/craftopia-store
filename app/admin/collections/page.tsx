...
import { Plus, Filter, ChevronRight, ChevronLeft, MoreHorizontal, Eye, Pencil, Trash2 } from "lucide-react";
import { CollectionFormDrawer, type CollectionFormData } from "@/components/admin/CollectionFormDrawer";
...
export default function AdminCollectionsPage() {
    ...
    const [editTarget, setEditTarget] = useState<Partial<CollectionFormData> | null>(null);
    ...
    const openEdit = (col: typeof INITIAL_COLLECTIONS[number]) => {
        setEditTarget({
            id: col.id,
            name: col.name,
            slug: col.slug,
    ...
    const handleSave = (data: CollectionFormData) => {
        if (data.id) {
            ...
            setCollections((cs) =>
                cs.map((c) =>
                    c.id === data.id
                        ? { ...c, name: data.name, slug: data.slug, status: data.status === "active" ? "Active" : "Inactive", updatedAt: "Just now" }
                        : c
                )
            );
        } else {
                        ...
                        <table className="w-full text-left border-collapse min-w-[700px]">
                            ...
                            <tbody className="divide-y divide-stone-100">
                                {collections.map((col) => {
                                    ...
                                    return (
                                        <tr key={col.id} className="hover:bg-stone-50/50 transition-colors bg-white group">
                                            ...
                                            <td className="py-4 px-6 text-right">
                                                <div className="flex justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <Link
                                                        ...
                                                        target="_blank"
                                                        className="w-8 h-8 flex items-center justify-center rounded-lg text-stone-400 hover:text-[#5CE614] hover:bg-stone-100 transition-colors"
                                                    >
