...
export function ProductFormDrawer({
  ...
  const addVariant = () =>
    setForm((f) => ({
      ...
      variants: [
        ...f.variants,
        { id: `v${Date.now()}`, name: "", price: "", stock: "", sku: "" },
      ],
    }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { uploadMultipleFiles } = await import("@/lib/api/client");

      // Filter out any invalid values from images array
      const validImages = form.images.filter(
        (img) => img && (typeof img === "string" || img instanceof File),
      );
      ...
      // Upload all new files in batch
      if (newFiles.length > 0) {
        try {
          const results = await uploadMultipleFiles(newFiles, "products");
      ...
      if (form.id) {
        ...
        onSave?.({ ...form, id: res.data._id, images: payload.images });
      } else {
      ...
      <form id="product-form" onSubmit={handleSubmit} className="space-y-5">
        ...
        {activeTab === "details" && (
          <>
            <FormSection title="Product Details">
              ...
              <FormField label="Slug (URL)" hint="/products/your-slug">
                <div className="flex items-center gap-2">
                  <span className="text-[12px] text-stone-400 font-medium whitespace-nowrap shrink-0">
                    /products/
                  </span>
                  <input
                    id="prod-slug"
            ...
            <FormSection
              title="Images"
              description="First image is the cover. Upload up to 4 images."
            >
              <MultiImageUpload
                files={form.images}
            ...
            <FormSection title="Organisation">
              ...
              <FormField
                label="Collection"
                hint="Parent collection this product belongs to."
              >
                <Select
                  id="prod-collection"
                ...
                >
                  ...
                  <option value="floral-arrangements">
                    Floral Arrangements
                  </option>
        ...
        {activeTab === "pricing" && (
          <>
              ...
              {discount !== null && (
                <div className="flex items-center gap-2 bg-[#FFF7ED] border border-orange-200 rounded-xl px-4 py-2.5">
                  ...
                  <p className="text-[12px] text-orange-700 font-medium">
                    Customer sees <strong>{discount}% off</strong> — compare-at
                    ${form.comparePrice} vs ${form.price}
                  </p>
                </div>
              )}
            </FormSection>
            ...
            <FormSection
              title="Variants"
              description="Offer sizes, colours, or other options."
            >
              <Toggle
                id="prod-variants"
              ...
              {form.hasVariants && (
                <div className="space-y-3 pt-1">
                  ...
                  {form.variants.map((v) => (
                    <div
                      key={v.id}
                      className="flex items-center gap-2 bg-stone-50 rounded-xl px-3 py-2.5 border border-stone-100"
                    >
                      ...
                      <div className="relative w-20 flex-shrink-0">
                        <span className="absolute left-2 top-1/2 -translate-y-1/2 text-stone-300 text-[12px]">
                          $
                        </span>
                        <input
                          type="number"
          ...
          <FormSection title="Inventory & Shipping">
            ...
            {form.trackInventory && (
              <div className="pl-[60px] space-y-3">
                ...
                <Toggle
                  id="continue-oos"
                  checked={form.continueWhenOOS}
                  onChange={(v) => set("continueWhenOOS", v)}
            ...
            <FormField
              label="Weight (grams)"
              hint="Used to calculate shipping rates."
            >
              <input
                id="prod-weight"
                type="number"
        ...
        {activeTab === "seo" && (
          <FormSection title="SEO & Metadata">
            <FormField
              label="SEO Title"
              hint="Defaults to the product title if empty."
            >
              <input
                id="prod-seo-title"
                type="text"
            ...
            <FormField label="Tags" hint="Press Enter or comma to add.">
              <TagInput
                ...
                placeholder="floral, white, roses…"
              />
            </FormField>
            ...
            {(form.seoTitle || form.title) && (
              <div className="rounded-xl border border-stone-200 p-4 bg-white space-y-1">
                ...
                <p className="text-[#006621] text-[12px]">
                  https://craftopia.com/products/{form.slug || "your-product"}
                </p>
