...
import { collectionsApi } from "@/lib/api/products";
...
export function CollectionFormDrawer({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.id) {
        await collectionsApi.update(form.id as string, payload);
      } else {
  ...
  return (
    <Drawer
      ...
      onClose={onClose}
      title={
        isEdit ? `Edit: ${initial?.name || "Collection"}` : "New Collection"
      }
      subtitle={
        isEdit
          ? "Update collection details and settings."
          : "Organise products into a browseable group."
      }
      footer={footer}
      width="max-w-xl"
    >
      <form id="collection-form" onSubmit={handleSubmit} className="space-y-5">
        ...
        <FormSection title="Basic Information">
          ...
          <FormField
            label="Description"
            hint="Displayed on the collection page."
          >
            <Textarea
              id="col-description"
        ...
        <FormSection title="SEO & Metadata">
          <FormField
            label="SEO Title"
            hint="Defaults to the collection name if empty."
          >
            <input
              id="col-seo-title"
              type="text"
